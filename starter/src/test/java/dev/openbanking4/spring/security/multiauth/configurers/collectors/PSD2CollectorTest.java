...
public class PSD2CollectorTest {
    ...
    private final String headerName = "x-cert";
    private final String testPSD2Certificate = "-----BEGIN CERTIFICATE-----\n" +
    ...
    public void setUp() {
        ...
        // Build a collector that will not return a username if the cert is not an eidas WEB cert.
        this.psd2Collector = PSD2Collector.psd2Builder()
                ...
                .psd2UsernameCollector((certificatesChain, psd2CertInfo) -> {
                    ...
                    if (psd2CertInfo != null && psd2CertInfo.isPsd2Cert()) {
                        Optional<EidasCertType> eidasCertType = psd2CertInfo.getEidasCertType();
                        if (eidasCertType.isPresent() || eidasCertType.get() == EidasCertType.WEB) {
                            if (eidasCertType.get() == EidasCertType.WEB) {
                                ...
                                log.debug("Expected eidasCertType to be WEB. It was '{}'. Will not collect username",
                                        eidasCertType.get());
                            }
                        } else {
                            log.debug("certificatesChain contains either null cert or non psd2 certificate");
                        }
                    } else {
                        log.debug("Certificate in request is not a psd2 eIDAS cert.");
                    }
                    return userName;
                })
                .psd2AuthoritiesCollector((certificatesChain, psd2CertInfo, roles) -> {
                    if (roles == null) {
    ...
    @Test
    public void psd2Collector_shouldFailToCollectIfNotEidasWebCert() {
        //Given
        HttpServletRequest mockedRequest = Mockito.mock(HttpServletRequest.class);
        RequestContextHolder.setRequestAttributes(new ServletWebRequest(mockedRequest));
